...
# MAIN PROGRAM DEFINED AS A FUNCTION
...
def main():
    ...
    # EYE DETECTOR OBJECT INSTANTIATION
    Eye_det = EyeDet(show_processing=args.show_eye_proc)

    # HEAD POSE ESTIMATOR OBJECT INSTANTIATION
    Head_pose = HeadPoseEst(show_axis=args.show_axis)

    # ATTENTION SCORER OBJECT INSTANTIATION
    ...
    # Get the date and time to name the datafile
    DT = datetime.now()
    ...
    with open(file_name, 'w', newline='') as csvfile:
        ...
        # MAIN WHILE LOOP FOR PROGRAM ACTIVE USE
        
        # infinite loop for webcam video capture
        while True:  

            delta_time = time.perf_counter() - prev_time  # delta time for FPS capping
            
            # VIDEOFRAME OBJECT INSTANTIATION
            ret, frame = cap.read()  # read a frame from the webcam

            # VIDEOFRAME ERROR HANDLING
            ...
            # IF STATEMENT TO COMPARE TRIGGER FRAME PROCESSING
            if delta_time >= time_lim:  # if the time passed is bigger or equal than the frame time, process the frame
                ...
                if len(faces) > 0:  # process the frame only if at least a face is found
                    ...
                    # WRITE HEAD POSE OBJECT VALUES (PITCH, ROLL, YAW) TO CSV
                    timestamp = time.time()
                    csvwriter.writerow([timestamp, pitch, roll, yaw])
                    csvfile.flush()  # Flush the buffer to ensure immediate writing


                        

                # DISPLAY SYSTEM PROCESSING TIME
